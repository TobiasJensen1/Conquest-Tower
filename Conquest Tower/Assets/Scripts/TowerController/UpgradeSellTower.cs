...
public class UpgradeSellTower : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetButtonDown("Fire1"))
        {
            ...
            {
                placedTowers[i].gameObject.transform.GetChild(4).GetComponent<SpriteRenderer>().enabled = false;
            }
            ...
            if (Physics.Raycast(ray, out hit))
            {
                ...
                if ((hit.transform.tag == "Tower" && CanUpgradenew) && (hit.collider.gameObject.transform.tag != "TurretHitbox"))
                {
                    ...
                    SpriteRenderer sprite = hit.collider.gameObject.transform.GetChild(4).GetComponent<SpriteRenderer>();
                    sprite.enabled = true;
                    CanBuild = false;
